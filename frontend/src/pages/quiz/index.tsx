...
export default function QuizPage() {
  ...
  return <View className='quiz-page'>
    ...
    <View className='quiz-bar'><Button className='icon-button' onClick={() => Taro.navigateBack()}>×</Button><Text>{String(currentIndex + 1).padStart(2, '0')} / 06</Text><Text className='timer'>02:18</Text></View>
    {feedback && <View className={`result-banner ${correct ? 'success' : 'danger'}`}><Text className='result-icon'>{correct ? '✓' : '×'}</Text><View><Text className='result-title'>{correct ? '回答正确' : '这道题答错了'}</Text><Text className='result-copy'>你的答案：{selected.join('、')} {correct ? '' : `· 正确答案：${question.answer.join('、')}`}</Text></View></View>}
    ...
    <View className='quiz-action'><Button className='primary-button' disabled={!selected.length && !feedback} onClick={feedback ? next : submit}>{feedback ? isLast ? '查看本轮报告' : '下一题 →' : '提交答案'}</Button></View>
  </View>
}
